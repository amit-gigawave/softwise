...
export const Navbar = () => {
    ...
    return (
        <header className="fixed flex z-50 w-full pt-6 px-4 top-0 left-0 justify-center pointer-events-none">
            <nav className="shadow-black/50 flex md:gap-12 md:w-auto bg-black/60 w-full max-w-5xl rounded-full py-2 pl-6 pr-2 shadow-2xl backdrop-blur-xl gap-x-8 gap-y-8 items-center justify-between pointer-events-auto border border-white/10 relative">
                {/* Logo */}
                <Link href="/" className="text-base font-medium text-white tracking-tight shrink-0">
                    Softwise.io
                </Link>
                ...
                <div className="hidden md:flex items-center gap-6">
                    {navItems.map((item) => (
                        <Link
                            key={item.name}
                            href={item.href}
                            className="hover:text-white transition-colors text-xs font-medium text-gray-400 tracking-wide"
                        >
                            {item.name}
                        </Link>
                    ))}
